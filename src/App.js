...
const ScalesBackground = () => (
  <div className="fixed inset-0 z-0 bg-[#050505] overflow-hidden pointer-events-none">
    ...
    {/* Glows - More focused center glow to back the text */}
    <div className="absolute top-[20%] left-[50%] -translate-x-1/2 w-[60%] h-[60%] bg-emerald-900/5 blur-[100px] rounded-full"></div>
    <div className="absolute bottom-0 w-full h-[30%] bg-gradient-to-t from-black to-transparent"></div>
    <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,transparent_0%,#000000_90%)]"></div>
  </div>
);
...
const TerminalSplash = ({ onComplete }) => {
  ...
  useEffect(() => {
    const sequence = [
      { text: "INITIALIZING PREDATOR PROTOCOL...", delay: 200 },
      { text: "LOADING VENOM MODULES...", delay: 800 },
      { text: "TAIPAN MEDIA: SUPREMACY ACHIEVED.", delay: 1500, color: "text-emerald-500 font-bold text-glow" },
    ];
    let timeouts = [];
  ...
  return (
    <div className="fixed inset-0 bg-black z-[100] flex flex-col justify-center items-center p-10 font-cinzel text-sm tracking-[0.2em] uppercase">
      <GlobalStyles />
      <div className="w-full max-w-md space-y-4">
        ...
        <div className="h-px w-32 bg-emerald-900/50 mt-8 animate-pulse"></div>
      </div>
    </div>
  );
};
...
const SelectView = ({ setMode }) => (
  <div className="flex flex-col h-screen relative overflow-hidden bg-black w-full">
    <GlobalStyles />
    <ScalesBackground />
    <div className="relative z-10 flex-1 flex flex-col p-8 justify-center items-center w-full">
      
      {/* Header */}
      <div className="mb-16 flex flex-col items-center w-full max-w-4xl mx-auto animate-in fade-in slide-in-from-top-10 duration-1000">
        
        {/* Crown Icon */}
        <div className="flex items-center gap-4 mb-8 opacity-50">
             <div className="h-px w-16 bg-gradient-to-r from-transparent to-emerald-500"></div>
             <Crown size={20} className="text-emerald-500 drop-shadow-[0_0_10px_rgba(16,185,129,0.5)]" strokeWidth={1} />
             <div className="h-px w-16 bg-gradient-to-l from-transparent to-emerald-500"></div>
        </div>
        
        {/* TITLE */}
        <h1 className="emerald-pulse-glow font-cinzel text-5xl md:text-7xl lg:text-8xl font-black tracking-widest leading-none mb-10 text-center select-none">
           TAIPAN<br/>
           <span className="text-white">MEDIA</span>
        </h1>

        {/* MISSION STATEMENT CONTAINER */}
        <div className="w-full relative glass-panel p-8 md:p-10 rounded-sm border-t border-b border-emerald-900/30">
           {/* Decorative corners */}
           <div className="absolute top-0 left-0 w-3 h-3 border-t border-l border-emerald-500/50"></div>
           <div className="absolute top-0 right-0 w-3 h-3 border-t border-r border-emerald-500/50"></div>
           <div className="absolute bottom-0 left-0 w-3 h-3 border-b border-l border-emerald-500/50"></div>
           <div className="absolute bottom-0 right-0 w-3 h-3 border-b border-r border-emerald-500/50"></div>

           <div className="flex flex-col items-center gap-8 text-center">
               {/* Main Tagline */}
               <p className="font-cinzel text-white text-lg md:text-2xl tracking-[0.1em] font-bold text-glow">
                 «МЫ СТРОИМ БИЗНЕС В TELEGRAM»
               </p>
               
               <div className="w-24 h-px bg-gradient-to-r from-transparent via-emerald-500/50 to-transparent"></div>

               {/* Subtitles Grid */}
               <div className="grid md:grid-cols-2 gap-8 w-full">
                  <div className="flex flex-col items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-emerald-900/20 flex items-center justify-center border border-emerald-500/20 mb-1">
                        <Hammer size={18} className="text-emerald-400" />
                      </div>
                      <p className="font-montserrat text-zinc-300 text-xs tracking-[0.1em] uppercase font-medium leading-relaxed max-w-xs">
                        <span className="text-emerald-500 font-bold">Бизнесу</span> — даем инструмент для сверхприбыли
                      </p>
                  </div>
                  ...
                  <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="font-montserrat text-zinc-300 text-xs tracking-[0.1em] uppercase font-medium leading-relaxed max-w-xs">
                        <span className="text-emerald-500 font-bold">Людям</span> — даем профессию, чтобы этот инструмент внедрять
                      </p>
                  </div>
               </div>
           </div>
      ...
      {/* Cards */}
      <div className="grid gap-6 w-full max-w-4xl md:grid-cols-2 px-4">
        <button 
            onClick={() => setMode('business')} 
            className="group relative bg-black border border-zinc-800 p-8 flex flex-col items-center text-center transition-all duration-500 hover:border-emerald-500/50 hover:bg-zinc-900/40"
        >
          <div className="absolute inset-0 bg-gradient-to-b from-emerald-500/0 via-emerald-500/0 to-emerald-500/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
          
          <Briefcase className="text-zinc-600 group-hover:text-emerald-400 transition-colors duration-500 mb-4" size={32} strokeWidth={1} />
          <h3 className="font-cinzel text-xl font-bold text-white uppercase tracking-widest mb-2 group-hover:text-emerald-400 transition-colors">Империя</h3>
          <p className="font-montserrat text-zinc-600 text-[10px] uppercase tracking-widest group-hover:text-zinc-400 transition-colors">Масштабирование • Власть</p>
        </button>
        ...
        >
          ...
          <Code className="text-zinc-600 group-hover:text-emerald-400 transition-colors duration-500 mb-4" size={32} strokeWidth={1} />
          <h3 className="font-cinzel text-xl font-bold text-white uppercase tracking-widest mb-2 group-hover:text-emerald-400 transition-colors">Создатель</h3>
          <p className="font-montserrat text-zinc-600 text-[10px] uppercase tracking-widest group-hover:text-zinc-400 transition-colors">Код • Эволюция • Контроль</p>
        </button>
      </div>
    </div>
  </div>
  ...
  if (step === 4) {
    return (
      <div className="flex flex-col h-screen bg-black text-white overflow-hidden relative w-full font-montserrat">
        ...
        <div className="relative z-10 flex flex-col h-full p-6 animate-in zoom-in-95 duration-1000 fade-in w-full">
           {/* Top Nav */}
           <div className="flex justify-between items-center mb-6 md:mb-8">
              <button onClick={() => setMode('select')} className="w-12 h-12 bg-black border border-zinc-800 flex items-center justify-center hover:border-emerald-500 hover:text-emerald-500 transition-colors">
                  <ArrowLeft size={20} />
              </button>
              <div className="px-4 py-2 border border-emerald-900/50 bg-emerald-950/20 text-[10px] font-bold text-emerald-500 uppercase tracking-[0.2em] backdrop-blur-sm font-cinzel">
                  Система Активна
              </div>
           </div>
           ...
           <div className="flex-1 flex flex-col justify-center items-center max-w-md mx-auto w-full text-center space-y-8">
              <div className="space-y-6 flex flex-col items-center">
                  <div className="relative inline-block group">
                      <div className="absolute inset-0 bg-emerald-500/10 blur-[80px] animate-pulse"></div>
                      <div className="w-24 h-24 mx-auto mb-6 relative z-10 group-hover:scale-105 transition-transform duration-700 ease-out">
                          <TelegramLogo />
                      </div>
                      <h1 className="font-cinzel text-5xl md:text-6xl font-black tracking-wide leading-none mb-2 text-white">
                          2026: <br/> <span className="text-emerald-500 text-glow">TG</span> МАРКЕТ
                      </h1>
                  </div>

                  <div className="space-y-6 flex flex-col items-center w-full">
                      <p className="text-zinc-400 font-montserrat text-xs uppercase tracking-[0.2em] leading-relaxed whitespace-nowrap mx-auto border-b border-zinc-800 pb-4">
                          Сайты — это прошлое. Приложения — это долго.
                      </p>
                      <div className="w-full bg-gradient-to-r from-transparent via-zinc-900 to-transparent p-px">
                        <div className="bg-black/40 backdrop-blur-md p-6 relative overflow-hidden text-center">
                             ...
                             <Quote className="mx-auto text-emerald-800 mb-2 opacity-50" size={16}/>
                             <p className="text-emerald-400 text-sm font-bold uppercase tracking-wide leading-relaxed font-cinzel">
              ...
              <div className="grid gap-4 w-full">
                  <button className="group relative bg-zinc-900/30 border border-zinc-800 hover:border-emerald-500/50 p-6 text-left transition-all duration-300">
                      ...
                      <div className="flex justify-between items-center">
                          <div>
                            <div className="text-xl font-bold font-cinzel text-white mb-1 group-hover:text-emerald-400 transition-colors tracking-wide">TG Магазины</div>
                            <p className="text-[10px] text-zinc-500 uppercase tracking-widest font-medium font-montserrat">Скрипты • Внедрение</p>
                          </div>
                          <ShoppingCart className="text-zinc-600 group-hover:text-emerald-500 transition-colors" size={20} strokeWidth={1.5}/>
                      </div>
                  </button>

                  <button className="group relative bg-zinc-900/30 border border-zinc-800 hover:border-purple-500/50 p-6 text-left transition-all duration-300">
                      ...
                      <div className="flex justify-between items-center">
                          <div>
                            <div className="text-xl font-bold font-cinzel text-white mb-1 group-hover:text-purple-400 transition-colors tracking-wide">Нейро-Боты</div>
                            <p className="text-[10px] text-zinc-500 uppercase tracking-widest font-medium font-montserrat">AI • Автоматизация</p>
                          </div>
                          <BrainCircuit className="text-zinc-600 group-hover:text-purple-500 transition-colors" size={20} strokeWidth={1.5}/>
                      </div>
                  </button>
              </div>
           </div>
  ...
  return (
    <div className="flex flex-col h-screen bg-black text-white font-montserrat overflow-hidden items-center justify-center relative p-6 w-full">
      ...
      <div key={step} className={`relative z-10 flex flex-col items-center text-center transition-all ${containerTransition}`}>
        <div className="mb-10 scale-110">{current.logo}</div>
        <div className="space-y-8 flex flex-col items-center">
            <h2 className={`text-9xl font-cinzel font-black tracking-tighter ${yearColor}`}>
                {current.year}
            </h2>
            <div className={`relative p-8 text-center backdrop-blur-md border-y ${quoteBorder} max-w-sm`}>
                <p className={`${isPositive ? 'text-white' : 'text-zinc-500'} text-xl font-cinzel font-bold uppercase leading-normal`}>
                    {current.quote}
                </p>
                <div className="mt-6 flex justify-center">
                    <span className={`text-[10px] uppercase tracking-[0.4em] ${footerColor} font-bold border-b border-current pb-1`}>{footerText}</span>
                </div>
            </div>
        </div>
      </div>
...
const BusinessView = ({ setMode, bizParams, setBizParams }) => {
        ...
        <div className="flex flex-col h-screen bg-black text-white font-montserrat items-center justify-center relative overflow-hidden p-6 w-full">
            ...
            <div className="relative z-10 flex flex-col items-center text-center space-y-8">
                <div className="relative">
                    ...
                    <BrainCircuit size={80} className="text-emerald-500 relative z-10 animate-bounce" strokeWidth={1} />
                </div>
                <div>
                    <h2 className="text-3xl font-cinzel font-bold tracking-widest text-white mb-3">НЕЙРОСКАН</h2>
                    <div className="flex flex-col gap-1 items-center">
                        <p className="text-emerald-600 text-[10px] font-bold uppercase tracking-[0.3em]">Поиск уязвимостей...</p>
                        <div className="h-0.5 w-24 bg-zinc-800 rounded-full overflow-hidden mt-2">
        ...
        <div className="flex flex-col h-screen bg-black text-white font-montserrat overflow-y-auto relative w-full">
            ...
            <div className="relative z-10 p-6 max-w-lg mx-auto w-full min-h-screen flex flex-col">
                <div className="flex items-center justify-between mb-8">
                    ...
                    <div className="text-emerald-500 border border-emerald-900/50 px-3 py-1 bg-emerald-950/30 text-[10px] font-bold uppercase tracking-widest font-cinzel">
                        Диагноз готов
                    </div>
                </div>
                ...
                <div className="flex-1 space-y-8">
                    <div className="text-center space-y-2">
                         <h2 className="text-4xl md:text-5xl font-cinzel font-black tracking-wider text-white">ПЛАН <span className="text-emerald-500 text-glow">ЗАХВАТА</span></h2>
                    </div>

                    <div className="bg-zinc-900/20 border-l-2 border-emerald-500 p-6 backdrop-blur-md relative overflow-hidden">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-zinc-900/30 p-6 border border-zinc-800 text-center transition-all hover:border-emerald-500/50 group">
                            <div className="text-[10px] text-zinc-500 uppercase font-bold mb-2 tracking-widest">Потенциал</div>
                            <div className="text-xl md:text-2xl font-cinzel font-bold text-emerald-500 group-hover:text-glow transition-all">+{formatCurrency(recoveredProfit)}</div>
                        </div>
                        <div className="bg-zinc-900/30 p-6 border border-zinc-800 text-center transition-all hover:border-red-900/50 group">
                            <div className="text-[10px] text-zinc-500 uppercase font-bold mb-2 tracking-widest">Кровопотеря</div>
                            <div className="text-xl md:text-2xl font-cinzel font-bold text-red-700 group-hover:text-red-600 transition-all">{formatCurrency(lostProfit)}</div>
                        </div>
                    </div>
                </div>

                <div className="mt-8 pt-6 border-t border-zinc-900/50 text-center">
                    <button onClick={() => setAnalysisState('input')} className="w-full bg-white text-black font-cinzel font-black py-4 uppercase tracking-[0.2em] hover:bg-emerald-400 transition-all hover:scale-[1.01]">
                        НОВАЯ ОХОТА
                    </button>
                </div>
            </div>
  ...
  return (
    <div className="flex flex-col h-screen bg-black text-white font-montserrat overflow-y-auto relative w-full">
      ...
      <div className="sticky top-0 z-40 px-6 py-4 bg-black/80 backdrop-blur-xl flex justify-between items-center border-b border-zinc-900">
      ...
      <div className="p-6 relative z-10 pb-32 max-w-lg mx-auto w-full">
        <div className="mb-12 text-center">
            <h2 className="text-4xl md:text-5xl font-cinzel font-black tracking-wide mb-3 text-white">
                АУДИТ <span className="text-emerald-600 text-glow">УПУЩЕННОЙ ПРИБЫЛИ</span>
            </h2>
            <p className="text-zinc-500 text-[10px] uppercase tracking-[0.3em] font-medium">Калькулятор упущенного доминирования</p>
        </div>

        <div className="relative">
            {/* Corner Accents */}
            <div className="absolute -top-2 -left-2 w-4 h-4 border-t border-l border-emerald-500/50"></div>
            <div className="absolute -top-2 -right-2 w-4 h-4 border-t border-r border-emerald-500/50"></div>
            <div className="absolute -bottom-2 -left-2 w-4 h-4 border-b border-l border-emerald-500/50"></div>
            <div className="absolute -bottom-2 -right-2 w-4 h-4 border-b border-r border-emerald-500/50"></div>

            <div className="p-1 bg-zinc-900/30 backdrop-blur-sm border border-zinc-800">
                <div className="p-6 grid grid-cols-2 gap-x-8 gap-y-10">
                    {[
                        ...
                        <div key={i} className="space-y-3 flex flex-col items-center text-center group">
                            <label className="text-[10px] font-bold text-zinc-500 uppercase flex items-center justify-center gap-2 w-full tracking-[0.2em] group-hover:text-emerald-500 transition-colors font-cinzel">
                            ...
                            <input 
                                ...
                                className="w-full bg-transparent border-b border-zinc-800 py-2 text-white text-xl font-cinzel font-bold focus:outline-none focus:border-emerald-500 text-center transition-all placeholder-zinc-800"
                                placeholder="0"
                            />
                        </div>
                    ))}
                ...
                <div className="bg-black/50 p-8 space-y-8 border-t border-zinc-800 mt-4">
                    <div className="text-center">
                        <div className="text-[10px] font-bold text-zinc-600 mb-2 uppercase tracking-[0.2em]">Текущий доход</div>
                        <div className="text-3xl font-cinzel font-bold text-white tracking-wide">
                            {formatCurrency(baseProfit)}
                        </div>
                    </div>

                    <div className="relative bg-red-950/10 border-l-2 border-red-900 p-6">
                        <div className="flex flex-col items-center text-center space-y-2">
                            <div className="flex items-center gap-2 text-red-600 font-bold uppercase text-[10px] tracking-[0.2em] animate-pulse">
                                <AlertTriangle size={14} /> Утечка капитала
                            </div>
                            <div className="text-[10px] text-zinc-500">
                                Упущенный трафик: <span className="text-zinc-300 font-bold">{lostPercent.toFixed(1)}%</span>
                            </div>
                            <div className="text-3xl font-cinzel font-bold text-red-800 opacity-90">
                    ...
                    <button 
                        onClick={handleAI} 
                        className="w-full bg-emerald-600 text-black font-cinzel font-black text-sm py-5 uppercase tracking-[0.2em] hover:bg-emerald-500 transition-all hover:shadow-[0_0_30px_rgba(16,185,129,0.3)]"
                    >
                          ЗАПУСТИТЬ ПРОТОКОЛ
                    </button>
                </div>
