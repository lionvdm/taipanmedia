...
class ErrorBoundary extends React.Component {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div style={{ padding: 20, color: 'red', background: 'black', minHeight: '100vh', wordWrap: 'break-word', fontFamily: 'monospace', zIndex: 99999, position: 'relative' }}>
          <h2 style={{fontSize: '20px', marginBottom: '10px'}}>Упс! Что-то сломалось.</h2>
          <p style={{marginBottom: '10px', color: '#999'}}>Покажите этот текст разработчику:</p>
          <pre style={{ whiteSpace: 'pre-wrap', fontSize: '12px', border: '1px solid #333', padding: '10px' }}>{this.state.error?.toString()}</pre>
        </div>
      );
    }
...
try {
  ...
  console.log("Using default config due to error reading env:", e);
}
...
const ProfitCalculator = ({ onAction, data, setData }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error("Telegram MainButton error:", e);
    }
  }, [onAction]);
...
const App = () => {
  ...
  const businessProfit = calculateProfit();
            ...
            <div className="w-full space-y-3">
                {userRole === 'business' ? (
                    <>
                        ...
                        {/* --- ИСПРАВЛЕННЫЙ БЛОК ЛИЧНОГО КАБИНЕТА (ВЕДЕТ В STRATEGY) --- */}
